...
| `packages/mcp` | **Implemented** | stdio + HTTP adapter over the catalog and installer. |
...
| ~~`packages/ai` (package.json + README, no source)~~ | — | **Superseded: removed.** The empty workspace was not zero-cost — it carried a lockfile importer and a build-context line in every Dockerfile. The seam is recorded in `docs/ai.md` instead. Do not implement; do not recreate the empty package. |
